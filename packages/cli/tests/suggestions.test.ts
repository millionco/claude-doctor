...
const makeProject = (name: string, signals: SignalResult[]): ProjectAnalysis => ({
  projectName: name,
...
describe("generateSuggestions", () => {
  it("generates rule for edit thrashing", () => {
    const projects = [
      makeProject("proj-a", [makeSignal("edit-thrashing", -20), makeSignal("edit-thrashing", -15)]),
    ];
    const suggestions = generateSuggestions(projects);
    expect(suggestions.some((rule) => rule.includes("Read the full file before editing"))).toBe(
      true,
    );
  });
  ...
  it("generates rule for user interrupts", () => {
    const projects = [
      makeProject("proj-a", [makeSignal("user-interrupts", -4), makeSignal("user-interrupts", -2)]),
    ];
    const suggestions = generateSuggestions(projects);
...
describe("generateAgentsRules", () => {
  it("outputs markdown with header", () => {
    ...
    expect(rulesText).toContain("## Rules (auto-generated by claude-doctor)");
