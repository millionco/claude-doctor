...
import { PI_SESSIONS_DIR } from "./constants.js";
...
export const discoverPiSessions = async (): Promise<PiSessionEntry[]> => {
  const sessionPaths: string[] = [];
  walkSessions(PI_SESSIONS_DIR, sessionPaths);

  const entries: PiSessionEntry[] = [];
  for (const sessionPath of sessionPaths) {
    const header = await readPiSessionHeader(sessionPath);
    if (!header) continue;
...
export const findLatestPiSession = async (
  projectFilter?: string,
): Promise<{ sessionPath: string; sessionId: string; cwd: string } | undefined> => {
  const sessions = await discoverPiSessions();
