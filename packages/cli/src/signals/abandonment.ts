...
export const detectAbandonment = (sessions: SessionMetadata[]): SignalResult[] => {
  ...
  for (let sessionIndex = 0; sessionIndex < sorted.length; sessionIndex++) {
    ...
    const timeSincePrevious = session.startTime.getTime() - previousSession.startTime.getTime();
  ...
  for (const cluster of clusters) {
    ...
    signals.push({
      ...
      severity:
        cluster.sessionIds.length >= RESTART_CLUSTER_CRITICAL_THRESHOLD ? "critical" : "high",
  ...
  if (
    shortSessionRatio > SHORT_SESSION_RATIO_THRESHOLD &&
    shortSessions.length >= MIN_SHORT_SESSIONS_TO_FLAG
  ) {
    signals.push({
